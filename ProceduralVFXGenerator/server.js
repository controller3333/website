/* server.js — zero-dependency static server + tiny settings API.
 *
 * Run:  node server.js [port]   (default 5601)
 *
...
const port = parseInt(process.argv[2] || '5601', 10);
...
const server = http.createServer((req, res) => {
  ...
  if (u.pathname === '/api/settings') {
    ...
    if (req.method === 'POST') {
      ...
      req.on('end', () => {
        try { JSON.parse(body); fs.writeFileSync(SETTINGS, body); res.writeHead(200, { 'Content-Type': 'application/json' }); res.end('{"ok":true}'); }
...
server.listen(port, () => console.log('Procedural VFX Generator -> http://localhost:' + port));
